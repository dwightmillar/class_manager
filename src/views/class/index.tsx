...
const ViewStudents = lazy(() => import("./students/index"));
const ViewAssignments = lazy(() => import("./assignments/index"));
const StudentView = lazy(() => import("../student/index"));
const AssignmentInput = lazy(() => import("../assignment/index"));
...
export default function (props: any) {
	...
	function calculateAssignmentAverages() {
		...
		Object.keys(assignments).forEach((title) => {
			let givenAssignment = state.assignments.filter(
			...
			assignments[title] =
				((scoredPoints / totalPoints) * 100).toFixed(2) + "%";
		});
	...
	function sortByGrade() {
		...
		students.sort((a: any, b: any) => {
			if (isNaN(parseFloat(state.studentAverages[a.id]))) return -1;
			if (isNaN(parseFloat(state.studentAverages[b.id]))) return 1;
			return parseFloat(state.studentAverages[a.id]) >
				parseFloat(state.studentAverages[b.id])
				? 1
				: -1;
		});
		let afterSort = JSON.stringify({ students });
	...
	function toggleView() {
		if (state.view === "assignments")
			return setState({ ...state, view: "students" });
		setState({ ...state, view: "assignments" });
	}

	const deleteStudentCallback: any = (studentId: any) => {
	...
	const addStudentCallback: any = (data: any, name: any) => {
				...
				render={(props) => (
					<>
						<Header
							...
							title={title}
							classId={classId}
							assignmentAverages={state.assignmentAverages}
						...
						{state.view === "students" ? (
							...
							<Suspense fallback={<div>Loading...</div>}>
								<ViewAssignments
									{...props}
									assignments={state.assignments}
									averages={state.assignmentAverages}
								/>
							</Suspense>
						)}
					</>
				...
				render={(props) => (
					<Suspense fallback={<div>Loading...</div>}>
						<AssignmentInput
							...
							students={state.students}
							studentAverages={state.studentAverages}
							classAverage={state.classAverage}
							classId={classId}
							title={title}
						/>
				...
				render={(props) => (
					<Suspense fallback={<div>Loading...</div>}>
						<StudentView
							...
							loadAssignments={loadAssignments}
							studentAverage={
								state.studentAverages[props.match.params.studentId]
							}
							assignments={state.assignments.filter(
								(assignment) =>
									assignment.student_id == props.match.params.studentId
							)}
						/>
					</Suspense>
				)}
